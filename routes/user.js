    ...
    login(req.body.username, req.body.password).then(data => {
        ...
        if (data && data.username) {
            if (identifying === req.session.captcha) {
                ...
                res.json(new resModels({ data: [], message: '登录成功！', status: 200 }))
                return
            } else {
...
router.get('/captcha', function (req, res, next) {
    ...
    if (cap.data) res.json(new resModels({ data: cap.data, status: 200, message: 'OK!' }))
    else res.json(new resModels({ data: [], status: 500, message: '获取验证码失败！' }))
});
...
router.post('/new', function (req, res, next) {
    if (req.session.auth === 1) {
        isRepeat(req.body.username)
            .then(data => {
                if (data.length > 0) {
                    res.json(new resModels({ data: [], message: '该用户已存在', status: 403 }))
                } else {
                    newUser(req.body)
                        .then(data => {
...
router.post('/delete', function (req, res, next) {
    if (req.session.auth === 1) {
        if (req.body.id.indexOf(req.session.userId) > -1) {
            res.json(new resModels({ data: [], message: '不能删除自己的账号！', status: 403 }))
        } else {
            deleteUser(req.body.id).then(data => {
                res.json(new resModels({ data, status: 200, message: 'OK!' }))
            }).catch(err => res.json(new resModels({ message: '参数不匹配！', status: 400 })))
        }
...
router.post('/update', function (req, res, next) {
    if (req.session.auth === 1) {
        updateUser(req.body).then(data => {
            ...
            res.json(new resModels({ data: err, message: '参数不匹配！', status: 400 }))
        })
    }
});
